...
getGeneGTF <- function(file){
  ...
  exonList <- IRanges::reduce(split(exonGTF, mcols(exonGTF)$gene_id))
...
getGeneGTF2 <- function(file){
  ...
  message("Computing Effective Exon Lengths...")
  exonGTF <- importGTF[importGTF$type=="exon",]
  exonList<-split(exonGTF, mcols(exonGTF)$gene_id)

  exonListGR<-GenomicRanges::GRangesList()
  for(i in seq_along(exonList)){
    exo<-exonList[[i]]
    name<-names(exonList[i])
    if(i%%100==0){
      cat(sprintf("No.%d  --- [ %s ]\n",i,name))
    }
    exonListGR[[name]]<-exo
  ...
  exonList <- GenomicRanges::reduce(exonListGR)
  exonReduced <- unlist(exonList, use.names=TRUE)
